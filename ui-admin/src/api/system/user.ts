...
// 获取当前用户常用菜单ID集合
export function getFrequentMenus(): Promise<AjaxResult<number[]>> {
  return request({
    url: '/system/user/profile/frequentMenus',
    method: 'get'
  })
}

// 更新当前用户常用菜单ID集合
export function updateFrequentMenus(menuIds: number[]): Promise<AjaxResult> {
  return request({
    url: '/system/user/profile/frequentMenus',
    method: 'put',
    data: menuIds
